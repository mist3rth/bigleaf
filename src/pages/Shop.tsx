    ...
    <div className="max-w-screen-2xl mx-auto px-4 md:px-8 py-8 text-primary animate-fade-in" id="product-list-page">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8 pb-6 border-b border-primary/10">
        <div>
          ...
          <p className="text-xs text-primary/50 font-mono tracking-widest uppercase mt-1">
        ...
        <aside className="lg:col-span-3 bg-secondary/20 rounded-2xl border border-primary/5 p-6 space-y-8 sticky top-28 text-left">
          ...
          <div className="flex items-center justify-between pb-4 border-b border-primary/10">
            ...
            {activeFiltersCount > 0 && (
              <button 
                onClick={handleReset}
                className="text-[11px] font-bold text-accent hover:text-primary transition-colors flex items-center gap-1 cursor-pointer"
              >
        ...
        <section className="lg:col-span-9 space-y-6">
          ...
          {activeFiltersCount > 0 && (
            <div className="flex flex-wrap items-center gap-2 py-2 text-left">
              <span className="text-[10px] font-mono tracking-wider text-primary/40 uppercase">Filtres actifs :</span>
              {selectedCategory !== 'all' && (
                <span className="bg-secondary text-primary border border-primary/10 px-2.5 py-1 rounded-full text-[10px] font-bold flex items-center gap-1.5">
                  ...
                  <button onClick={() => setSelectedCategory('all')} className="hover:text-accent font-black">×</button>
                </span>
              )}
              {selectedCollection && (
                <span className="bg-accent/15 text-accent border border-accent/20 px-2.5 py-1 rounded-full text-[10px] font-bold flex items-center gap-1.5">
                  ...
                  <button onClick={() => setSelectedCollection(null)} className="hover:text-accent font-black">×</button>
                </span>
              )}
              {selectedDifficulty !== 'all' && (
                <span className="bg-secondary text-primary border border-primary/10 px-2.5 py-1 rounded-full text-[10px] font-bold flex items-center gap-1.5">
                  Entretien : {selectedDifficulty}
                  <button onClick={() => setSelectedDifficulty('all')} className="hover:text-accent font-black">×</button>
                </span>
              )}
              {selectedLight !== 'all' && (
                <span className="bg-secondary text-primary border border-primary/10 px-2.5 py-1 rounded-full text-[10px] font-bold flex items-center gap-1.5">
                  Exposition : {selectedLight}
                  <button onClick={() => setSelectedLight('all')} className="hover:text-accent font-black">×</button>
                </span>
              )}
              {priceRange < 100 && (
                <span className="bg-secondary text-primary border border-primary/10 px-2.5 py-1 rounded-full text-[10px] font-bold flex items-center gap-1.5">
                  Max : {priceRange} €
                  <button onClick={() => setPriceRange(100)} className="hover:text-accent font-black">×</button>
                </span>
              )}
              {search !== '' && (
                <span className="bg-secondary text-primary border border-primary/10 px-2.5 py-1 rounded-full text-[10px] font-bold flex items-center gap-1.5">
                  Recherche : "{search}"
                  <button onClick={() => setSearch('')} className="hover:text-accent font-black">×</button>
                </span>
              )}
            </div>
          )}
          ...
          {processedProducts.length === 0 ? (
            <div className="py-24 text-center bg-secondary/10 rounded-2xl border border-primary/5 shadow-inner">
              <RotateCcw className="w-12 h-12 text-primary/30 mx-auto mb-4 animate-spin-slow" />
              <p className="font-display text-xl font-bold text-primary/75 uppercase">Aucun résultat botanique</p>
              <p className="text-sm text-primary/50 mt-2 max-w-sm mx-auto">
